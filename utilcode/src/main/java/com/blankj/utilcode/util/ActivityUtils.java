...
public final class ActivityUtils {
    ...
    /**
     * @param context
     * @param safePackages 白名单
     * @return true表示安全（没被劫持）、false表示不安全
     * @purpose 检测Activity是否被劫持
     */
    public static boolean checkAntiHijacking(Context context, List<String> safePackages) {
        boolean safe = false;
        ActivityManager activityManager = (ActivityManager) context
                .getSystemService(Context.ACTIVITY_SERVICE);
        String runningActivityPackageName = activityManager.getRunningTasks(1)
                .get(0).topActivity.getPackageName();
        if (runningActivityPackageName.equals(context.getPackageName())) {
            safe = true;
        }
        // 白名单比对
        if (safePackages != null) {
